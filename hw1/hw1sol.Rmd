...
Elizabeth is the most mentioned by a noticeable margin. Note that the counts differ slightly when compared with the results from `grep -c`, though the conclusions remain the same:

```{bash,eval=F}
grep -c -e Eliza -e Lizzy pride_and_prejudice.txt
grep -c Jane pride_and_prejudice.txt
grep -c Lydia pride_and_prejudice.txt
grep -c Darcy pride_and_prejudice.txt
```

Adding the counts for "Eliza" and "Lizzy" gets us the totals:
Elizabeth: 751
Jane: 290
Lydia: 166
Darcy: 414
